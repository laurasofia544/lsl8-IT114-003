...
public class ChatView extends JPanel implements IMessageEvents, IConnectionEvents, IRoomEvents {
        ...
        button.addActionListener(_ -> {
            SwingUtilities.invokeLater(() -> {
                try {
                    ...
                    if (!text.isEmpty()) {
                        ClientConsole.INSTANCE.sendMessage(text);
                        textValue.setText("");
                    }
    ...
    public void onMessageReceive(long clientId, String message) {
        ...
        String displayName = ClientConsole.INSTANCE.getDisplayNameFromId(clientId);
    ...
    public void onClientDisconnect(long clientId) {
        ...
        boolean isMe = ClientConsole.INSTANCE.isMyClientId(clientId);
        String message = String.format("*%s disconnected*",
                isMe ? "You" : ClientConsole.INSTANCE.getDisplayNameFromId(clientId));
        addText(message);
    }
