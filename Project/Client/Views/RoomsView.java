...
public class RoomsView extends JPanel implements IRoomEvents {
        ...
        searchButton.addActionListener(_ -> {
            SwingUtilities.invokeLater(() -> {
                try {
                    ...
                    if (!query.isEmpty()) {
                        removeAllRooms();
                        ClientConsole.INSTANCE.sendRoomAction(query, RoomAction.LIST);
        ...
        createButton.addActionListener(_ -> {
            SwingUtilities.invokeLater(() -> {
                try {
                    String query = searchValue.getText().trim();
                    if (!query.isEmpty()) {
                        ClientConsole.INSTANCE.sendRoomAction(query, RoomAction.CREATE);
        ...
        joinButton.addActionListener(_ -> {
            SwingUtilities.invokeLater(() -> {
                try {
                    String query = searchValue.getText().trim();
                    if (!query.isEmpty()) {
                        ClientConsole.INSTANCE.sendRoomAction(query, RoomAction.JOIN);
    ...
    public void handleSelection(String room) {
        SwingUtilities.invokeLater(() -> {
            try {
                ClientConsole.INSTANCE.sendRoomAction(room, RoomAction.JOIN);
            } catch (IOException e) {
