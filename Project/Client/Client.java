...
public enum Client {
    ...
    /**
     * Used for client-side feedback into the Game Events panel
     *
     * @param str
     */
    public void clientSideGameEvent(String str) {
        passToUICallback(IMessageEvents.class,
                e -> e.onMessageReceive(Constants.GAME_EVENT_CHANNEL, str));
    }
    ...
    private boolean processClientCommand(String text) throws IOException {
        ...
        if (text.startsWith(Constants.COMMAND_TRIGGER)) {
            ...
            || text.equalsIgnoreCase("ready")) {
                ...
                if (arg.isEmpty()) {
                    LoggerUtil.INSTANCE.warning(
                            TextFX.colorize("Usage: /pick r|p|s", Color.RED));
                    return true;
                }
                sendPick(arg);
                wasCommand = true;
    ...
    private void processPayload(Payload payload) {
        switch (payload.getPayloadType()) {
            ...
            //lsl8 | 12/08/25 | Round Start + Picked + Result + Game Over messages
            case ROUND_START:
                processMessage(payload);
            ...
            default:
                LoggerUtil.INSTANCE.warning(TextFX.colorize(
                        "Unhandled payload type: " + payload.getPayloadType(), Color.YELLOW));
                break;
